...
LOG_FILE=/tmp/runtool.sh.${mypid}.log
...
then
  ...
  then
      echo "start dot command to PNG format...." >> $LOG_FILE
      echo `date` >> $LOG_FILE
      dot -Tjpeg test$mypid.$suffix.dot -o /tmp/test$mypid.jpeg
      dot -Tpdf test$mypid.$suffix.dot -o /tmp/test$mypid.pdf
      echo "ending the dot commands ..." >> $LOG_FILE
      echo `date` >> $LOG_FILE
      echo "Available formats of DotGenerator tool: <br>"
      echo "<a href=\"/imgs/test$mypid.jpeg\" target=\"_blank\">.JPEG</a> "
      echo "<a href=\"/imgs/test$mypid.pdf\" target=\"_blank\">.PDF</a> "
      echo"<p>"
  elif [ $tool == "dotGeneratorWholeASTGraph" ]
  then
      dot -Tjpeg test$mypid.${suffix}_WholeAST.dot -o /tmp/test${mypid}_WholeAST.jpeg
      ...
      echo "<a href=\"/imgs/test${mypid}_WholeAST.jpeg\" target=\"_blank\">.JPEG</a> "
  ...
  then
      dot -Tjpeg test$mypid.${suffix}_callGraph.dot -o /tmp/test${mypid}_callGraph.jpeg
      ...
      echo "<a href=\"/imgs/test${mypid}_callGraph.jpeg\" target=\"_blank\">.JPEG</a> "
